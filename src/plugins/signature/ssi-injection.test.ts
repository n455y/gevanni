...
import { SsiInjectionPlugin } from "./ssi-injection.ts";
...
describe("SsiInjectionPlugin", () => {
  it("creates definitions only for parameters with AppendValue mutation", async () => {
    ...
    const targets = [
      makeQueryParameter("name", "test"),
      makeJsonPrimitiveParam(["page"], "index"),
    ];
  ...
  it("detects SSI directive execution in response body", async () => {
    const plugin = new SsiInjectionPlugin();
    await plugin.init({
      commandBus,
      eventBus: new InMemoryEventBus(),
  ...
  it("does not report vulnerability when SSI is not executed", async () => {
    const plugin = new SsiInjectionPlugin();
    await plugin.init({
      commandBus,
      eventBus: new InMemoryEventBus(),
