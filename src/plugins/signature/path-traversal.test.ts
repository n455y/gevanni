...
import { PathTraversalPlugin, PATH_TRAVERSAL_PATTERNS } from "./path-traversal.ts";
...
describe("PathTraversalPlugin", () => {
  it("creates definitions only for parameters with ReplaceValue mutation", async () => {
    ...
    expect(items[0].signatureName).toBe("path-traversal");
  });
  ...
  it("includes all required path traversal patterns", () => {
    ...
    expect(
      PATH_TRAVERSAL_PATTERNS[1].test("[extensions]\r\nmauto=1"),
    ).toBe(true);
    expect(
      PATH_TRAVERSAL_PATTERNS[2].test("; for 16-bit app support"),
    ).toBe(true);
  });
});
